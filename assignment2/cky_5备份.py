            ...
            for s2 in self.matrix[mid][end].labels():
                if (s1._symbol,s2._symbol) in self.binary:
                    for s in self.binary[(s1._symbol,s2._symbol)]:
                        ...
                        label = Label(s,(s1._symbol,s2._symbol),[(start,mid),(mid,end)])
                        cell.addLabel(label)
                        #cell.unaryUpdate(s,1)
    # def createUnaryTree(self, unaryTree, label):
        # if isinstance(label, type(None)):
            # return unaryTree
        # if label._children == None:
            # tree = str(label._symbol)
            # return tree
        # if isinstance(label._children, nltk.Nonterminal) or isinstance(label._children,str):
            # label_children = None
            # for la in self.matrix[label._children_locations[0][0]][label._children_locations[0][1]].labels():
                # if la._symbol == label._children:
                    # label_children = la
            # tree = "(" + str(label._symbol) + " " + self.createUnaryTree(tree, label_children) + ")"
            # return tree
    def createTree(self, tree, label):
        ...
        if isinstance(label._children, nltk.Nonterminal) or isinstance(label._children,str):
            ...
            for la in self.matrix[label._children_locations[0][0]][label._children_locations[0][1]].labels():
                if la._symbol == label._children:
                    label_children = la
            tree = "(" + str(label._symbol) + " " + self.createTree(tree, label_children) + ")"
            return tree
        if isinstance(label._children,tuple):
            label_children1 = None
            label_children2 = None
            tree1 = ''
            tree2 = ''
            for la1 in self.matrix[label._children_locations[0][0]][label._children_locations[0][1]].labels():
                for la2 in self.matrix[label._children_locations[1][0]][label._children_locations[1][1]].labels():
                    if la1._symbol in label._children and la2._symbol in label._children:
                        tree1 = self.createTree(tree1, la1)
                        tree2 = self.createTree(tree2, la2)
    ...
    def unaryUpdate(self,label,depth=0,recursive=False):
        ...
        if label._symbol in self.matrix.unary:
            for parent in self.matrix.unary[label._symbol]:
                ...
                label = Label(parent,label._symbol,children_locations=label._children_locations)
                self.addLabel(label)
...
class Label:
    ...
    class'''
    def __init__(self,symbol,children=None, children_locations=None
        ...
        '''
        self._symbol=symbol
        self._children = children#store the parents of this label
    ...
    #add an attribute to label to store its parents
    def children(self):
    ...
    def set_children_locations(self,locations):
        for location in locations:
            self._children_locations.append(location)
